...
### Слабое железо / много параллельных сессий

Каждый сеанс — это отдельный процесс `opencode serve` (~200–400MB), поэтому
на слабой машине десятки одновременно живущих сессий лучше не держать
горячими. Для этого есть три ручки:

1. **Квота** `VIBEPROD_MAX_CONCURRENT_SESSIONS` (например `10`): сверх лимита
   сессии ждут в очереди (`status=queued`) и стартуют по мере освобождения
   слотов. 100 сессий работают по очереди, а не одновременно.
2. **Двухуровневый idle**: после `VIBEPROD_SUSPEND_IDLE_MIN` минут простоя
   контейнер паузится (`docker pause` — 0 CPU), после `VIBEPROD_IDLE_TTL_MIN`
   — убивается. Просыпание паузы ~1с, холодный старт ~10–30с (образ уже
   локально).
3. **Ресурсы воркера**: `VIBEPROD_WORKER_MEM`/`_SWAP`/`_PIDS` — заниженный
   `mem_limit` плюс swap позволяют упаковать больше контейнеров.

На Linux-проде дополнительно помогает KSM (kernel same-page merging) —
дедупликация одинаковых страниц памяти между однотипными процессами:

```bash
echo 1 > /sys/kernel/mm/ksm/run
```

(на macOS/Docker Desktop недоступен — там работают только пункты 1–3).
...
**Память агента** — долговременный текст, который агент видит в каждой новой
сессии (инжектится в agent-файл) и обновляет сам инструментами `memory_get`/
`memory_set` встроенного MCP «vibeprod». Память включается по умолчанию при
создании агента и отключается в карточке агента (вкладка «Память»).
...
**Автоматизации по событиям** (Автоматизация → «По событиям») запускают агента
прямо внутри брокера при событии: сессия завершилась или упала, сработало
расписание, пришёл вебхук. Промпт правила — шаблон: в нём доступны `{event}`,
`{json}` (полный пейлоад события) и поля события — `{title}`, `{status}`,
`{prompt}`, `{agent_name}`, `{project_name}`, `{error}`, `{url}`, `{session_id}`.
Например, правило «после каждого упавшего запуска сообщи в Telegram причину и
предложи фикс» — это агент с промптом `Событие {event}. {json}`, подписанный на
`session.failed`.

Каждый запуск правила — обычная сессия с меткой `automation`, а результат
виден в журнале запусков правила. Правила можно собирать в цепочки («завершилась
сессия A — стартует сессия B»), но по умолчанию каскады выключены: правило не
реагирует на сессии, созданные другими автоматизациями, — это защищает от
случайных бесконечных циклов. Для осознанных цепочек включите «каскады» в
настройках правила.
...
запусков по расписанию, вебхукам и автоматизациям — на каждый запуск или только
при ошибке.
...
Простой трекер задач: название, детальное описание, дата, статус
(открыт / в работе / готово) и теги, с поиском и фильтрами. Заводить можно
вручную в интерфейсе или руками агентов — у каждого агента есть встроенные
инструменты `issue_create`, `issue_list`, `issue_update`, `issue_delete`:
агент нашёл проблему или сделал задачу — записал её в трекер, вы увидели
в интерфейсе.
...
| `VIBEPROD_IDLE_TTL_MIN` | `60` | минут простоя до убийства воркера (история живёт в docker-томе, воркер поднимается заново при следующем сообщении) |
| `VIBEPROD_SUSPEND_IDLE_MIN` | `15` | минут простоя до паузы воркера (`docker pause`: 0 CPU, память остаётся). Должно быть меньше `VIBEPROD_IDLE_TTL_MIN` |
| `VIBEPROD_MAX_CONCURRENT_SESSIONS` | `0` | квота параллельно живущих воркеров; сверх лимита сессии ждут в очереди (`status=queued`). `0` — без ограничений. Для слабого железа: `10`–`20` |
| `VIBEPROD_WORKER_MEM` | `384m` | `mem_limit` воркера |
| `VIBEPROD_WORKER_SWAP` | `1024m` | `memswap_limit` воркера: простаивающие страницы уходят на диск |
| `VIBEPROD_WORKER_PIDS` | `512` | `pids_limit` воркера (защита от fork-бомб) |
